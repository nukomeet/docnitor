...
Cuba.define do
  on get do
    on root do
      ...
      client['nukomeetment'].send('Docnitor', render("messages/overdue.slim", projects: overdue), :color => color(overdue.count))
      client['nukomeetment'].send('Docnitor', render("messages/not_estimated.slim", projects: not_estimated), :color => color(not_estimated.count))
      client['nukomeetment'].send('Docnitor', render("messages/not_scheduled.slim", projects: not_scheduled), :color => color(not_scheduled.count))
